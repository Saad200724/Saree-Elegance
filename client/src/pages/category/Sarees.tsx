    ...
    >
      ...
      <section className="relative h-[25vh] md:h-[40vh] w-full overflow-hidden rounded-b-3xl bg-gray-900">
        ...
        <div className="relative h-full max-w-7xl mx-auto px-4 flex flex-col justify-center items-center text-white text-center">
          <h1 className="text-3xl md:text-5xl font-bold mb-2 md:mb-4 font-heading">Sarees</h1>
          <p className="text-sm md:text-lg max-w-2xl opacity-90 px-4">Timeless elegance woven into every thread.</p>
        </div>
      </section>
        ...
        {isLoading ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
            {[1,2,3,4,5,6,7,8].map(i => (
              <div key={i} className="h-48 md:h-96 bg-gray-100 animate-pulse rounded-xl" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
